...
extern blocked_pool_t gl_blocked_th_pool;/*Thread pool for Wheel Timer*/
#define SCHEDULER_THREAD_POOL_SIZE	1/*One because Scheduler will be just one sible thread*/
...
ppm_setup_scheduler(){
	...
	assert(!is_setup);
	is_setup = 1;
	init_blocked_pool(&gl_blocked_th_pool, SCHEDULER_THREAD_POOL_SIZE);
...
ppm_schedule_outbound_rule(const char *proto_name, ppm_outbound_rule_t *ppm_outbound_rule){
	...
	for(; i < GET_OIF_COUNT(ppm_outbound_rule); i++){
		...
		wt_elem = register_app_event(ppm_get_scheduler(), 
